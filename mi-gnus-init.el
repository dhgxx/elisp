...
(setq
 ...
 gnus-default-directory "~/emacs/gnus"
 gnus-article-save-directory "~/emacs/gnus/news/articles"
 gnus-kill-files-directory "~/emacs/gnus/news/articles"
 message-directory "~/emacs/gnus/messages"
 message-auto-save-directory "~/emacs/gnus/messages/autosave"
 mail-source-directory message-directory
 nndraft-directory "~/emacs/gnus/messages/drafts")
...
(defun mi-header-on-wrote-cn ()
  ...
  (let ((sc-mumble "")
    ...
    (if whofrom
		(progn
		  (setq mi-whofrom-id (replace-regexp-in-string "\\(\\.bbs\\|@.*\\|.*[<]\\|[>].*\\)" "" whofrom))
		  (setq mi-whofrom-nick (replace-regexp-in-string "\\(.*\(\\|\).*\\|\"\\|[ ]*<.*>\\)" "" whofrom))
		  (if (and
...
(setq sc-citation-leader ""
      ...
      sc-citation-delimiter-regexp "\\([>]+\\|\\(\\w\\|[_.]\\)+[:]+\\)")
